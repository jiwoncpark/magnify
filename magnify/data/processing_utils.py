...
def normalize_masked_data(data, mask, att_min, att_max):
    """Normalize masked data to lie in a fixed range of [-1, 1]

    Parameters
    ----------
    data : torch.tensor
        Data (y values) that will be masked, of shape [B, T, Y_dim]
    mask : torch.tensor
        Mask that has 1 where observed 0 otherwise, of shape [B, T, Y_dim]
    att_min : torch.tensor
        Global minimum values for y, of shape [Y_dim]
    att_max : torch.tensor
        Global maximum values for y, of shape [Y_dim]
...
def subsample_observed_data(data_dict, n_tp_to_sample=None,
    ...
    """Subsample observed vs. predict from the available y time series

    Parameters
    ----------
    data_dict : dict
        Data dict processed by split_data_extrap/split_data_interp
    n_tp_to_sample : None, optional
        randomly subsample the time points. The resulting timeline has
        n_tp_to_sample points
    n_points_to_cut : None, optional
        cut out consecutive points on the timeline.  The resulting
        timeline has (N - n_points_to_cut) points
...
def subsample_timepoints(data, time_steps, mask, n_tp_to_sample=None):
    # n_tp_to_sample: number of time points to subsample. If not None,
    # sample exactly n_tp_to_sample points
    ...
    if n_tp_to_sample > 1:
        ...
        for i in range(data.size(0)):
            ...
            subsampled_idx = sorted(np.random.choice(non_missing_tp,
                                                     n_to_sample,
                                                     replace=False))
