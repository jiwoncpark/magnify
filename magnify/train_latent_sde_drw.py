...
def make_drw_data():
    ...
    ts_ = ts_vis_[random_idx]
    ys_ = ys_vis_[random_idx, None]
    ts_ext_ = np.array([0.0] + list(ts_) + [ts_vis_[-1]])

    import matplotlib.pyplot as plt
    plt.scatter(ts_vis_, ys_vis_)
    plt.scatter(ts_, ys_)
        ...
        if global_step % args.pause_iters == 0:
            ...
            with torch.no_grad():
                ...
                if args.show_arrows:
                    num, dt = 12, 0.12
                    t, y = torch.meshgrid(
                        [torch.linspace(ts_vis_[0], ts_vis_[-1], num).to(device),
                         torch.linspace(-1, 1, num).to(device)]
                    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='drw', choices=['segmented_cosine', 'irregular_sine', 'drw'])
    parser.add_argument('--kl-anneal-iters', type=int, default=100, help='Number of iterations for linear KL schedule.')
    parser.add_argument('--train-iters', type=int, default=500, help='Number of iterations for training.')
    parser.add_argument('--pause-iters', type=int, default=10, help='Number of iterations before pausing.')
