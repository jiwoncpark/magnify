...
def compute_param_mse_loss(label_predictions, labels):
    """

    Args:
        label_predictions: torch.tensor of shape [n_traj_samples, batch_size, Y_dim]
        labels: torch.tensor of shape [batch_size, Y_dim]

    Returns:

    """
    n_traj_samples = label_predictions.shape[0]
    labels = labels.unsqueeze(0).repeat(n_traj_samples, 1, 1)
    return mse_loss(label_predictions, labels, reduction='mean')
