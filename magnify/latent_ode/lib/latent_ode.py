    ...
    def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps,
        ...
        if isinstance(self.encoder_z0, Encoder_z0_ODE_RNN) or \
           isinstance(self.encoder_z0, Encoder_z0_RNN):
            ...
            # Encoding is done backwards in time, so approx posterior over
            # latent is defined on the first point
            first_point_mu, first_point_std = self.encoder_z0(
                truth_w_mask, truth_time_steps, run_backwards = run_backwards)  # [1, batch_size, dim_latent]

            means_z0 = first_point_mu.repeat(n_traj_samples, 1, 1)  # [n_traj_samples, batch_size, dim_latent]
            sigma_z0 = first_point_std.repeat(n_traj_samples, 1, 1)  # [n_traj_samples, batch_size, dim_latent]
            first_point_enc = utils.sample_standard_gaussian(means_z0, sigma_z0)  # [n_traj_samples, batch_size, dim_latent]
        ...
        if self.use_regressor:
            all_extra_info["label_predictions"] = self.regressor(first_point_enc)
