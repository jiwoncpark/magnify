...
parser.add_argument('--dataset', type=str, default='periodic', help="Dataset to load. Available: drw, physionet, activity, hopper, periodic")
...
parser.add_argument('--regress', action='store_true', help="Include regression loss -- used for periodic dataset for predicting frequency")
...
if __name__ == '__main__':
    ...
    if args.rnn_vae:
        ...
        model = RNN_VAE(input_dim, args.latents,
                        ...
                        obsrv_std = obsrv_std,
                        z0_prior = z0_prior,
                        predict_params= args.classif or args.regress,
        ...
        z0_diffeq_solver = DiffeqSolver(input_dim, rec_ode_func, "euler",
                                        args.latents,
                                        odeint_rtol=1e-3,
                                        odeint_atol=1e-4,
                                        device=device)
        ...
        model = ODE_RNN(input_dim, n_ode_gru_dims, device=device,
                        ...
                        classif_per_tp=classif_per_tp,
                        n_labels=n_labels,
                        train_classif_w_reconstr=(args.dataset in ["physionet", "drw"])
                        ).to(device)
    ...
    for itr in range(1, num_batches * (args.niters + 1)):
        ...
        train_res = model.compute_all_losses(batch_dict,
                                             n_traj_samples=3,
                                             kl_coef=kl_coef)
        ...
        if itr % (n_iters_to_viz * num_batches) == 0:
            with torch.no_grad():
                ...
                if "mse_param_loss" in test_res:
                    logger.info("MSE param loss: {}".format(test_res["mse_param_loss"]))
